...
# ─────────────────────────────────────────────────────────────────────────────
USERS = {
    "jvilla": {"password": "valmex",   "nombre": "José Carlos Villa", "iniciales": "JC", "rol": "admin"},
...
ISIN_MAP = {
  ...
  "VXTBILL": {"A":"MX51VA1F0004","B0CO":"MX51VA1F0020","B0FI":"MX51VA1F0012","B0NC":"MX51VA1F0053","B1CF":"MX51VA1F0087"},
...
FONDO_TIPO = {
    "VXREPO1": "deuda", "VXGUBCP": "deuda", "VXUDIMP": "deuda", "VXDEUDA": "deuda",
    "VXGUBLP": "deuda", "VXTBILL": "deuda", "VXCOBER": "deuda",
    "VLMXETF": "rv_libre", "VLMXDME": "rv_libre", "VALMX28": "rv_libre",
    "VLMXTEC": "rv_libre", "VLMXESG": "rv_libre", "VALMXHC": "rv_libre", "VXINFRA": "rv_libre",
    "VALMXA": "rv_asesorado", "VALMX20": "rv_asesorado", "VALMXVL": "rv_asesorado", "VALMXES": "rv_asesorado",
    "VLMXJUB": "ciclo", "VLMXP24": "ciclo", "VLMXP31": "ciclo", "VLMXP38": "ciclo",
    "VLMXP45": "ciclo", "VLMXP52": "ciclo", "VLMXP59": "ciclo",
}

# ─────────────────────────────────────────────────────────────────────────────
# REGLAS DE SERIE POR TIPO DE CLIENTE Y TIPO DE FONDO
# Los valores del select "Serie del Cliente" en Configuración deben
# coincidir EXACTAMENTE con las keys de estos dicts.
# ─────────────────────────────────────────────────────────────────────────────
SERIE_RULES = {
    "deuda": {
        "Persona Física - B1FI/B1":           "B1FI",
        "Persona Física con Fee - B0FI/B0":    "B0FI",
        "Plan Personal de Retiro - B1NC/B1CF": "B1NC",
        "Persona Moral - B1CO":                "B1CO",
    },
    "rv_asesorado": {
        "Persona Física - B1FI/B1":           "B1",
        "Persona Física con Fee - B0FI/B0":    "B0",
        "Plan Personal de Retiro - B1NC/B1CF": "B1",
        "Persona Moral - B1CO":                "B1",
    },
    "ciclo": {
        "Persona Física - B1FI/B1":           "B1FI",
        "Persona Física con Fee - B0FI/B0":    "B0FI",
    ...
    "rv_libre": {
        "Persona Física - B1FI/B1":           "B1FI",
        "Persona Física con Fee - B0FI/B0":    "B0FI",
        "Plan Personal de Retiro - B1NC/B1CF": "B1CF",
        "Persona Moral - B1CO":                "B1CO",
    },
}

# Caché en memoria del universo Morningstar (se llena la primera vez)
_ms_cache = {}
...
def load_ms_universe():
    """Carga el universo completo una vez y lo guarda en caché."""
    if _ms_cache:
        return _ms_cache
    try:
        resp = requests.get(MS_URL,
                            params={"accesscode": MS_ACCESS, "format": "JSON"},
                            timeout=25)
        ...
        print(f"[MS] Universo cargado: {len(_ms_cache)} fondos en caché")
...
def resolve_serie(fondo, tipo_cliente):
    """Devuelve la serie correcta para un fondo según el tipo de cliente."""
    tipo    = FONDO_TIPO.get(fondo, "deuda")
    reglas  = SERIE_RULES.get(tipo, SERIE_RULES["deuda"])
    deseada = reglas.get(tipo_cliente, "B1FI")
    disponibles = ISIN_MAP.get(fondo, {})
    if deseada in disponibles:
        return deseada
    # Fallback en orden de preferencia
    for fb in ["B1FI", "B0FI", "B1NC", "B1CF", "B1CO", "B1", "B0", "A"]:
...
def calcular_portafolio(fondos_pct: dict, tipo_cliente: str) -> dict:
    """Pondera datos de Morningstar según la cartera recibida."""
    ...
    for fondo, pct in fondos_pct.items():
        ...
        # Fallback: primera serie disponible del fondo
        if not d:
            for s in ["B1FI", "B0FI", "B1NC", "B1CF", "B1CO", "B1", "B0", "A"]:
                t2 = f"{fondo} {s}"
                if t2 in universe:
                    d = universe[t2]
                    serie = s
    ...
    def top_n(d, n=8):
        ...
        return {"labels": [i[0] for i in items],
                "values": [round(i[1] / t * 100, 2) for i in items]}
    ...
    return {
        ...
        "deuda": {
            "gov_bond":  round(gov_t,  2),
            "corp_bond": round(corp_t, 2),
            "duracion":  round(dur_t,  4),
            "ytm":       round(ytm_t,  4),
    ...
    if request.method == "POST":
        ...
        if user and user["password"] == p:
            ...
            return jsonify({"ok": True, "nombre": user["nombre"],
                            "iniciales": user["iniciales"], "rol": user["rol"]})
...
def api_propuesta():
    ...
    Body JSON:
      {
        "tipo_cliente": "Persona Física - B1FI/B1",
        "modo": "perfil" | "actual" | "propuesta",
        "perfil_id": "3",               ← solo si modo=perfil
        "fondos": {"VXGUBCP": 20.4}    ← solo si modo=actual|propuesta
      }
    """
    if "usuario" not in session:
    ...
    result = calcular_portafolio(fondos_pct, tipo_cliente)
